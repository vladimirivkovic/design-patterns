...
public class TableBuilderDemoAfter {
	public static void main(String[] args) {
		(new TableBuilderDemoAfter()).demo(args);
	}
	...
	public void demo(String[] args) {
		...
		Builder target = null;
		try {
			target = (Builder) Class.forName(getClass().getName() + "$" + class_name).getDeclaredConstructor().newInstance();
		} catch (Exception ex) {
            ex.printStackTrace();
        }
